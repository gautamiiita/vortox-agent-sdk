...
/**
 * Requires a shared API key on every {@code /agent/**} request.
 *
...
public class SidecarApiKeyFilter extends OncePerRequestFilter {
    ...
    public SidecarApiKeyFilter(@Value("${sidecar.api-key:}") String apiKey,
        ...
        if (!authEnabled) {
            ...
            log.warn("sidecar.auth-enabled=false — /agent/** is UNAUTHENTICATED. Anything that can "
            ...
            log.error("sidecar.api-key is not set — every /agent/** request will be refused. "
                    + "Set SIDECAR_API_KEY to enable the sidecar.");
        }
    }

    /** Only /agent/** is protected; health checks and static assets are not. */
    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        return !authEnabled || !request.getRequestURI().startsWith("/agent");
    }
